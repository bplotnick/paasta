...
def test_emit_resource_requirements(tmpdir):
    ...
    with mock.patch(
        ...
        mock_clusterman_metrics.generate_key_with_dimensions.side_effect = lambda name, dims: (
            f'{name}|spark_framework_name={dims["spark_framework_name"]}'
        )

        emit_resource_requirements(spark_config_dict, 'anywhere-prod')

        mock_clusterman_metrics.ClustermanMetricsBotoClient.assert_called_once_with(region_name='us-north-14')
        metrics_writer = mock_clusterman_metrics.ClustermanMetricsBotoClient().get_writer().__enter__()

        metric_key_template = 'spark_requested_{resource}|spark_framework_name=paasta_spark_run_johndoe_2_3'
