...
def kubernetes_app_deploy_status_human(status, backoff_seconds=None):
    ...
    elif status == kubernetes_tools.KubernetesDeployStatus.Delayed:
    ...
    elif status == kubernetes_tools.KubernetesDeployStatus.Stopped:
...
def print_kubernetes_status(
    ...
    if status != KubernetesDeployStatus.NotRunning:
        if status == KubernetesDeployStatus.Delayed:
            deploy_status = kubernetes_app_deploy_status_human(status, kubernetes_status.backoff_seconds)
        else:
